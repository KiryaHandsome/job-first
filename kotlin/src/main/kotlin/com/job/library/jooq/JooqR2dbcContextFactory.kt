...
class JooqR2dbcContextFactory(
    ...
    fun <T> fetchManyAndAwait(
        mapper: RecordMapper<T>,
        block: DSLContext.() -> Select<org.jooq.Record>
    ): List<T> {
        val dslContext = DSL.using(connectionFactory)
    ...
    fun <T> fetchPageAndAwait(
        ...
        val totalCountRecord = Mono.from(dslContext.select(count().`as`("total_count")).from(table)).block()
    ...
    fun use(block: DSLContext.() -> DMLQuery<out org.jooq.Record>) {
        val dslContext = DSL.using(connectionFactory)
